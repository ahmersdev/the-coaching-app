...
export default function WorkoutOne({ control, watch, dayIndex }: any) {
  const { fields, append, remove } = useFieldArray({
    control,
    name: `daysAll[${dayIndex}].daysAllWorkoutOneReps`,
  });

  const watchSets = watch(`daysAll[${dayIndex}].sets`);
              ...
              <Grid container spacing={1}>
                {fields?.map((rep: any, repIndex: number) => (
                  <Grid item xs={12} md={2.5} key={rep.id}>
                    <RHFTextField
                      name={`daysAll[${dayIndex}].daysAllWorkoutOneReps[${repIndex}].rep`}
